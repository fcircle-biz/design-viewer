...
| 業務フロー | 担当者・システムのレーンと業務フェーズの列に作業・分岐を並べ、流れを矢印で結ぶ | `modes.biz` |
...
### 5. 目で確認する
- `<アプリ>/docs/spec/html/index.html` を開き、6 モード（キー 1〜6）を全体表示（F）で見る。
...
幅は次回開いたときも保持する）、詳細パネルの［画面を開く］で等倍プレビュー、［近傍のみ］で選択ノードの隣接だけ表示、
右下のミニマップで移動（詳細パネル表示中に可視領域が狭いと自動的に隠れる）、グループ見出しクリックでそのグループに寄る、
データフローは右上のステップで段階ごとに強調。
...
- 業務フローの列（フェーズ内の位置）はフェーズ内の最長経路順で決まる。ルートは直交で他ノードを避ける候補から選ぶが、辺が多いと通路で重なることがある。1 フェーズ 7 ノード程度を目安に分ける。
